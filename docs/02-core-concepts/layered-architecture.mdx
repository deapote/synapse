...
WebFlux 是 Reactive 的（Mono/Flux），但 Application 层是同步的。这是怎么做到的？
...
- [ ] 依赖方向是什么？为什么不能反向依赖？
- [ ] Domain 层的四个组成部分是什么？
- [ ] Application 层的入站端口和出站端口有什么区别？
- [ ] 为什么叫"端口适配器"模式？用电脑的比喻解释。
- [ ] 为什么 Application 层是同步的，而不是 Mono/Flux？
- [ ] Config 层的职责是什么？它做不做业务逻辑？
