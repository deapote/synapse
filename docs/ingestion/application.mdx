...
文件：`synapse-kb-application/.../port/service/DocumentCommandService.java`
...
由 `DocumentGovernanceService` 实现。
...
由 `DocumentIndexRefreshService` 执行。
...
由 `DocumentIndexRefreshService` 处理。索引刷新失败不会自动回退 Mongo 状态。`Document.indexStatus` 变为 `FAILED`，`lastIndexFailureReason` 记录失败原因。恢复方式：
