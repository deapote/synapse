---
title: Adapter-In 层
description: DocumentController、IngestionJobWorker、文件上传处理、metadata 参数
---

# 文档上传与摄入 —— Adapter-In 层

## DocumentController

文件：`synapse-kb-adapter/.../in/web/DocumentController.java`

```java
@RestController
public class DocumentController {
    private final IngestDocumentUseCase ingestUseCase;
    private final UpdateDocumentMetadataUseCase updateMetadataUseCase;
    private final SupersedeDocumentUseCase supersedeUseCase;
    // ... 其他 use case

    @PostMapping(value = "/api/knowledge-bases/{kbId}/documents",
                 consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public Mono<DocumentResponse> upload(
            @PathVariable String kbId,
            @RequestPart("file") FilePart filePart,
            @RequestParam(required = false) String sourceType,
            @RequestParam(required = false) String canonicalKey,
            @RequestParam(required = false) String versionLabel,
            @RequestParam(required = false) String effectiveFrom,
            @RequestParam(required = false) String effectiveTo,
            @RequestParam(required = false) String supersedesDocumentId,
            @RequestParam(required = false) Integer authorityLevel,
            @RequestParam(required = false) String jurisdiction) {

        return DataBufferUtils.join(filePart.content(), maxJoinBytes())
            .map(dataBuffer -> {
                byte[] bytes = new byte[dataBuffer.readableByteCount()];
                dataBuffer.read(bytes);
                DataBufferUtils.release(dataBuffer);
                return bytes;
            })
            .onErrorMap(DataBufferLimitException.class,
                e -> new DomainException("文件大小超过限制", e))
            .flatMap(bytes -> SaTokenReactorBridge.blockingCall(() -> {
                // 文件校验
                if (bytes.length <= 0 || bytes.length > maxFileBytes) {
                    throw new DomainException("文件大小超过限制");
                }
                String fileName = safeFileName(filePart.filename());
                String contentType = filePart.headers().getContentType() != null
                    ? filePart.headers().getContentType().toString()
                    : "application/octet-stream";
                validateFile(fileName, contentType);
                String contentHash = bytesToHex(
                    MessageDigest.getInstance("SHA-256").digest(bytes)
                );

                // 解析 metadata
                DocumentMetadata metadata = parseMetadata(
                    sourceType, canonicalKey, versionLabel,
                    effectiveFrom, effectiveTo,
                    supersedesDocumentId, authorityLevel, jurisdiction
                );

                // 构建命令并执行
                IngestDocumentCommand command = new IngestDocumentCommand(
                    new KnowledgeBaseId(kbId), fileName,
                    contentType, bytes.length, contentHash,
                    new ByteArrayInputStream(bytes), metadata
                );

                DocumentId documentId = ingestUseCase.ingest(command);
                return toResponse(documentId, kbId, fileName, contentType, bytes.length);
            }));
    }
}
```

### Metadata 参数说明

上传接口支持以下 `@RequestParam` 参数：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `sourceType` | string | 否 | `LEGAL`、`POLICY`、`GENERAL` |
| `canonicalKey` | string | 否 | 规范标识，用于同一资料的多版本关联 |
| `versionLabel` | string | 否 | 版本标签，如 "2024 版" |
| `effectiveFrom` | string | 否 | 生效日期，格式 `YYYY-MM-DD` |
| `effectiveTo` | string | 否 | 排他结束日期，格式 `YYYY-MM-DD` |
| `supersedesDocumentId` | string | 否 | 被替代的旧文档 ID |
| `authorityLevel` | int | 否 | 权威等级，数值越高优先级越高 |
| `jurisdiction` | string | 否 | 适用区域/管辖范围 |

### v2 治理接口

```java
@PutMapping("/api/documents/{id}/metadata")
public Mono<DocumentResponse> updateMetadata(@PathVariable String id,
                                              @RequestBody PatchDocumentMetadataRequest request) {
    // 调用 UpdateDocumentMetadataUseCase.updateMetadata()
    // PatchValue 语义：不传 = unset，传值 = set，传 null = clear
}

@PostMapping("/api/documents/{id}/supersede")
public Mono<Void> supersede(@PathVariable String id,
                            @RequestBody SupersedeDocumentRequest request) {
    // 调用 SupersedeDocumentUseCase.supersede()
    // 旧文档必须是 ACTIVE，新旧文档必须属于同一知识库
}

@PostMapping("/api/documents/{id}/retire")
public Mono<DocumentResponse> retire(@PathVariable String id,
                                     @RequestBody RetireDocumentRequest request) { ... }

@PostMapping("/api/documents/{id}/reactivate")
public Mono<DocumentResponse> reactivate(@PathVariable String id) { ... }

@PostMapping("/api/documents/{id}/reindex")
public Mono<DocumentResponse> reindex(@PathVariable String id) { ... }

@GetMapping("/api/documents/{id}/version-chain")
public Mono<List<DocumentVersionChainResponse>> getVersionChain(@PathVariable String id) { ... }

@GetMapping("/api/documents/{id}/audit-events")
public Mono<List<DocumentAuditEventResponse>> getAuditEvents(@PathVariable String id) { ... }
```

所有治理操作都会将 `indexStatus` 标记为 `STALE`，由后台 `DocumentIndexRefreshJobWorker` 异步刷新索引。刷新完成后 `indexStatus` 变为 `SYNCED`。

### 文件校验

校验项：
- 文件大小（默认 20MB）
- 文件扩展名（pdf, doc, docx, txt, md）
- Content-Type
- SHA-256 内容去重

## IngestionJobWorker

文件：`synapse-kb-adapter/.../in/worker/IngestionJobWorker.java`

```java
@Component
@EnableScheduling
public class IngestionJobWorker implements DisposableBean {
    private final ProcessIngestionJobUseCase processUseCase;
    private final ExecutorService executorService;
    private final Semaphore permits;
    private final AtomicBoolean enabled;
    private final String workerId;

    public IngestionJobWorker(ProcessIngestionJobUseCase processUseCase,
                              @Value("${synapse.ingestion.job.enabled:true}") boolean enabled,
                              @Value("${synapse.ingestion.job.concurrency:2}") int concurrency,
                              @Value("${synapse.ingestion.virtual-threads:true}") boolean virtualThreads) {
        this.processUseCase = processUseCase;
        this.executorService = virtualThreads
            ? Executors.newVirtualThreadPerTaskExecutor()
            : Executors.newFixedThreadPool(concurrency);
        this.permits = new Semaphore(concurrency);
        this.enabled = new AtomicBoolean(enabled);
        this.workerId = ManagementFactory.getRuntimeMXBean().getName() + "-" + UUID.randomUUID();
    }

    @Scheduled(fixedDelayString = "${synapse.ingestion.job.poll-delay-ms:1000}")
    public void poll() {
        if (!enabled.get()) return;

        while (permits.tryAcquire()) {
            executorService.execute(() -> {
                try {
                    boolean processed = processUseCase.processNextAvailable(workerId);
                } catch (Exception e) {
                    log.error("摄入任务执行失败", e);
                } finally {
                    permits.release();
                }
            });
        }
    }

    @Override
    public void destroy() {
        executorService.shutdown();
        try {
            if (!executorService.awaitTermination(30, TimeUnit.SECONDS)) {
                executorService.shutdownNow();
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            executorService.shutdownNow();
        }
    }
}
```

### Worker 设计要点

**定时轮询**：`@Scheduled(fixedDelay = 1000)` 每 1 秒执行一次 `poll()`。

**并发控制**：`Semaphore` 限制同时处理的任务数（默认 2）。`tryAcquire()` 非阻塞获取许可，获取不到就不提交新任务。

**虚拟线程**：`Executors.newVirtualThreadPerTaskExecutor()` 创建虚拟线程池。每个任务一个虚拟线程，创建成本极低。

**优雅关闭**：`DisposableBean.destroy()` 确保应用关闭时，等待正在执行的任务完成（最多 30 秒），然后强制关闭。
