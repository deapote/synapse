---
title: Domain 层
description: Document 实体、状态机、时效元数据、索引同步状态、IngestionJob、RecursiveChunkingStrategy
---
...
### DocumentIndexStatus —— 索引同步状态（v2）

文件：`synapse-kb-domain/.../model/DocumentIndexStatus.java`
...
`DocumentIndexStatus` 独立于摄入状态机和业务时效状态。所有治理操作（patch metadata、supersede、retire、reactivate、reindex）都会将 `indexStatus` 设为 `STALE`，由后台 Worker 异步刷新。

### metadataVersion / indexedMetadataVersion（v2）

- `metadataVersion`：每次 metadata 变更（patch、supersede、retire、reactivate、reindex）时递增
- `indexedMetadataVersion`：当前索引成功刷新时对应的 `metadataVersion`
- 当 `metadataVersion > indexedMetadataVersion` 时，`indexStatus` 应为 `STALE` 或 `REFRESHING`

Worker 在 claim job 后会检查 `document.getMetadataVersion() == job.getMetadataVersion()`，不匹配则跳过旧 job。
...
**关键设计**：先计算所有候选值，校验全部通过后再统一赋值。避免部分字段已修改、部分校验失败的中间状态。

### markIndexStale() / markIndexRefreshing() / markIndexSynced() / markIndexFailed()
...
`sourceType` 和 `jurisdiction` 可作为检索时的扩展过滤字段（v2 中已在列表筛选和检索过滤中启用）。

## PatchValue<T> —— v2 三态值对象

文件：`synapse-kb-domain/.../model/PatchValue.java`
...
用于在线 metadata 编辑，区分三种语义：
- `unset()` —— 不传该字段，不修改
- `set(T)` —— 覆盖为新值
- `clear()` —— 显式清空该字段（`sourceType` 和 `effectiveFrom` 不允许 `clear`）

## PatchDocumentMetadata —— v2 在线编辑值对象

文件：`synapse-kb-domain/.../model/PatchDocumentMetadata.java`
...
## DocumentIndexRefreshJob —— v2 索引刷新任务

文件：`synapse-kb-domain/.../model/DocumentIndexRefreshJob.java`
...
public class DocumentIndexRefreshJob {
    ...
    private final KnowledgeBaseId knowledgeBaseId;
    private final long metadataVersion;  // 触发时的 metadata 版本
...
文件：`synapse-kb-domain/.../repository/DocumentChunkRepository.java`
...
摄入完成后，chunks 被持久化到 Mongo `document_chunks` 集合。索引刷新时不需要重新解析原文档，直接从持久化 chunks 重建索引。这是两阶段索引刷新的基础。
