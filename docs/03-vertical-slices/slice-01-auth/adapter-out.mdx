...
- [ ] `LoginSessionPort` 和 `StpUtil` 的关系是什么？
- [ ] 如果想从 Sa-Token 换成 JWT，需要改哪些文件？
- [ ] BCrypt 为什么需要 salt？
- [ ] `SaTokenPermissionAdapter` 是什么时候被调用的？
- [ ] 双层 Repository 模式的两层分别负责什么？
- [ ] `toDocument()` 和 `toEntity()` 需要做哪些类型转换？
